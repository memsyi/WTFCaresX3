...
public class ControlerWrapper : MonoBehaviour 
{
	...
	public bool X_Hit(int index)
	{
		return(padCurrent[(int)index].Buttons.X ==  ButtonState.Pressed ) && (padLast[(int)index].Buttons.X == ButtonState.Released);	
	}
	
	public bool X_Hold(int index)
	{
		return padCurrent[index].Buttons.X == ButtonState.Pressed;
	}
